...
app.post('/api/certificates/save', (req, res) => {
  try {
    const { registrationNumber, studentName, studentCategory, certification, image, id } = req.body;
    ...
    // استخدم ID المرسل أو أنشئ واحداً جديداً
    const certId = id || Date.now().toString();
    const fileName = `${certId}_${registrationNumber}_${studentName.replace(/\s+/g, '_')}`;
    
    // حذف الملفات القديمة لنفس الشهادة (إذا كانت موجودة)
    try {
      const files = fs.readdirSync(dataDir);
      files.forEach(file => {
        if (file.startsWith(certId + '_')) {
          try {
            fs.unlinkSync(path.join(dataDir, file));
            console.log(`تم حذف الملف القديم: ${file}`);
          } catch(e) {
            console.warn(`فشل حذف: ${file}`, e.message);
          }
        }
      });
    } catch(e) {
      console.warn('خطأ في البحث عن الملفات القديمة:', e.message);
    }
